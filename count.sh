...
zsp_parser_ast_h=$(cat $(find packages/zuspec-parser/build/zsp_ast/src -name '*.h') | wc -l)
zsp_parser_ast_cpp=$(cat $(find packages/zuspec-parser/build/zsp_ast/src -name '*.cpp') | wc -l)
zsp_parser_py=$(cat $(find packages/zuspec-parser/python -name '*.p*') | wc -l)
zsp_parser_ast_py=$(cat $(find packages/zuspec-parser/build/zsp_ast/ext -name '*.p*') | wc -l)

count=`expr $count + $zsp_parser_h`
count=`expr $count + $zsp_parser_cpp`
count=`expr $count + $zsp_parser_ast_h`
count=`expr $count + $zsp_parser_ast_cpp`
count=`expr $count + $zsp_parser_py`
count=`expr $count + $zsp_parser_ast_py`
